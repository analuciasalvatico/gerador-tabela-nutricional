...
st.markdown("Preencha os dados abaixo e baixe a imagem PNG gerada.")
...
for nutriente in nutrientes:
    qtde = st.text_input(f"{nutriente} - Quantidade", "")
    vd = st.text_input(f"{nutriente} - %VD", "")
    if qtde or vd:
...
for extra, incluir in extras.items():
    if incluir:
        qtde = st.text_input(f"{extra} - Quantidade", "")
        vd = st.text_input(f"{extra} - %VD", "")
...
if st.button("Gerar Imagem PNG"):
    ...
    draw.text((10, 10), f"INFORMAÇÃO NUTRICIONAL", fill="black", font=fonte)
    ...
    st.image(byte_im, caption="Imagem gerada (400x300 px)", use_column_width=False)
